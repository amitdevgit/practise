...
//**** Slide Panel Toggle ***//
	...
	// $("span.mobile-menu").on("click", function(){
	// 	alert('ok');
	// 	$(".res-menu").toggleClass('active');
	// 	$(".theme-layout").toggleClass('active');
		
	// });
  	// $('body.active').on("click",function(e){
	// 	  alert('ok');
	// 	  if ($(e.target).closest('.side-panel').length===0) {
	// 		$(this).removeClass('active');
	// 		$(".side-panel,.res-menu").removeClass('active');
	// 	  }
...
$(document).mouseup(function(e) 
{
    ...
    if (!container.is(e.target) && container.has(e.target).length === 0 && !foot.is(e.target) && foot.has(e.target).length === 0) 
    {
        	$(".res-menu,.side-panel").removeClass('active');
